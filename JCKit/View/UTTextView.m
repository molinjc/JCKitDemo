...
@interface UTTextView ()

@property (nonatomic, strong) UILabel *placeholderLabel;

@end

@implementation UTTextView

- (instancetype)init {
    if (self = [super init]) {
        [self addSubview:self.placeholderLabel];
        [self addTextViewTextObserver];
    }
    return self;
}
...
- (void)updatePlaceholderLabelHeight {
    ...
    CGSize size = [string boundingRectWithSize:CGSizeMake(self.placeholderLabel.bounds.size.width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.placeholderLabel.font} context:nil].size;
    CGFloat height = 22;
