...
#define ARG_GET(type) do { type val = 0; [self getArgument:&val atIndex:(NSInteger)index]; return @(val); } while (0)
...
- (id)argumentAtIndex:(NSUInteger)index {
    ...
    if (strcmp(argType, @encode(id)) == 0 || strcmp(argType, @encode(Class)) == 0) {
        ...
        ARG_GET(unsigned char);
    } else if (strcmp(argType, @encode(unsigned int)) == 0) {
        ARG_GET(unsigned int);
    } else if (strcmp(argType, @encode(unsigned short)) == 0) {
        ARG_GET(unsigned short);
...
- (NSArray *)allArguments {
    ...
    return argumentsArray.mutableCopy;
}
