...
- (void (^)(id))actionBlock {
    _JCUIBarButtonItemBlockTarget *target = objc_getAssociatedObject(self, @selector(setActionBlock:));
    return target.block;
}
...
- (instancetype)initWithImage:(UIImage *)image style:(UIBarButtonItemStyle)style actionBlock:(void (^)(id sender))actionBlock {
    _JCUIBarButtonItemBlockTarget *target = [[_JCUIBarButtonItemBlockTarget alloc] initWithBlock:actionBlock];
    objc_setAssociatedObject(self, @selector(setActionBlock:), target, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
