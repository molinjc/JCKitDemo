...
#define __alwaysInline __inline__ __attribute__((always_inline))
            ...
            [relationMapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyName, NSString *mappedToKey, BOOL *stop) {
                ...
                if (propertyMeta) {
                    ...
                    if (keyPath.count > 1) {
                        ...
                        _JCModelPropertyMeta *mapper_meta = mapper[keyPath.firstObject];
                        if (mapper_meta) {
                            while (mapper_meta -> _next) {
                                mapper_meta = mapper_meta -> _next;
                            }
                            mapper_meta -> _next = propertyMeta;
                        }else {
...
static __alwaysInline NSDateFormatter *JCISODateFormatter() {
...
/** runtime发送信息(调用方法) */
static __alwaysInline void messageSend(__unsafe_unretained id model,  SEL setter, id value){
    if (setter) {
...
/** 获取NSBlock class */
static __alwaysInline Class JCNSBlockClass() {
    static Class cls;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        void (^block)(void) = ^{};
...
static __alwaysInline NSNumber *JCNSNumberCreateFromID(__unsafe_unretained id value) {
...
/** 用get方法获取值 */
static __alwaysInline NSNumber *JCModelCreateNumberFromProperty(__unsafe_unretained id model,
...
/** 处理数据类型 */
static __alwaysInline void JCModelSetNumberToProperty(__unsafe_unretained id model,
...
static void JCTypeObjectHandel(__unsafe_unretained id _value, _JCModelPropertyMeta *meta, __unsafe_unretained id model) {
    ...
    if (value) { messageSend(model, meta->_setter, value); }
}
...
static id JCModelToJSONObjectRecursive(id model) {
    ...
    if ([model isKindOfClass:[NSDictionary class]]) {
        ...
        [((NSDictionary *)model) enumerateKeysAndObjectsUsingBlock:^(NSString *key, id obj, BOOL *stop) {
            ...
            id object = JCModelToJSONObjectRecursive(obj);
            if (!object) { object = (id)kCFNull; }
            
            values[stringKey] = object;
        }];
        return values;
    }
    ...
    if ([model isKindOfClass:[NSDate class]]) { return [JCISODateFormatter() stringFromDate:model]; }
    ...
    [modelMeta -> _mapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyMappedKey, _JCModelPropertyMeta *propertyMeta, BOOL *stop) {
        ...
        if (propertyMeta -> _isCNumber) {
            value = JCModelCreateNumberFromProperty(model, propertyMeta);
        } else if (propertyMeta -> _nsType) {
            id obj = ((id (*)(id, SEL))(void *) objc_msgSend)((id)model, propertyMeta->_getter);
            value = JCModelToJSONObjectRecursive(obj);
        } else {
            switch (propertyMeta->_type & JCEncodingTypeMask) {
                ...
                case JCEncodingTypeClass: {
                    Class obj = ((Class (*)(id, SEL))(void *) objc_msgSend)((id)model, propertyMeta->_getter);
                    value = obj ? NSStringFromClass(obj) : nil;
                } break;
                case JCEncodingTypeSEL: {
                    SEL obj = ((SEL (*)(id, SEL))(void *) objc_msgSend)((id)model, propertyMeta->_getter);
                    value = obj ? NSStringFromSelector(obj) : nil;
                } break;
                default: break;
            }
        }
...
- (void)modelSetWithDictionary:(NSDictionary *)dictionary {
    ...
    if (meta -> _keyMappedCount == 0) { return; }
